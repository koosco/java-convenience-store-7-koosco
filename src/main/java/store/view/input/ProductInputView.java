...
public class ProductInputView {
    ...
    public List<ProductOrderRequestDto> purchaseProducts(List<Stock> stocks) {
    ...
    private List<ProductOrderRequestDto> convertToRequest(List<Stock> stocks, String products) {
        List<ProductOrderRequestDto> dtos = converter.toRequest(products);
    ...
    private static class Validator {
        ...
        public void validateProducts(List<Stock> stocks, List<ProductOrderRequestDto> dtos) {
            for (ProductOrderRequestDto dto : dtos) {
                Stock findStock = validateName(stocks, dto);
                validateQuantity(dto, findStock);
            }
        }

        private static Stock validateName(List<Stock> stocks, ProductOrderRequestDto dto) {
        ...
        private static void validateQuantity(ProductOrderRequestDto dto, Stock findStock) {
    ...
    private static class Converter {
        ...
        public List<ProductOrderRequestDto> toRequest(String products) {
            return Arrays.stream(products.split(PRODUCT_TOKEN_DELIMITER))
                ...
                .map(item -> {
                    ...
                    // 상품 이름, 상품 개수
                    return new ProductOrderRequestDto(parts[0], Integer.parseInt(parts[1]), false);
                }).toList();
        }
    }
}
